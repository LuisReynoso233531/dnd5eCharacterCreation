...
class CreateCharacterViewModel extends ChangeNotifier {
  ...
  List<String> get racialWeaponProficiencies =>
      _extractWeaponProfsFromTraits((_selectedRace?['traits'] ?? '').toString());
  ...
  Future<void> fetchFeats() async {
    ...
    try {
      _allFeats =
          List<Map<String, dynamic>>.from(await _repository.getFeats());
      notifyListeners();
    } catch (e) {
      _errorMessage = 'Error al cargar Feats: $e';
    } finally {
  ...
  static const _knownWeapons = [
    'battleaxe', 'handaxe', 'light hammer', 'warhammer', 'longsword',
    'shortsword', 'shortbow', 'longbow', 'spear', 'crossbow', 'dagger',
    'rapier', 'scimitar', 'greataxe', 'greatsword', 'trident', 'javelin',
    'quarterstaff', 'simple weapons', 'martial weapons', 'hand crossbow',
    'light crossbow', 'heavy crossbow', 'war pick', 'flail', 'mace',
  ];

  static const _knownTools = [
    "artisan's tools", "thieves' tools", "herbalism kit", "poisoner's kit",
    "disguise kit", "forgery kit", "navigator's tools", "alchemist's supplies",
    "brewer's supplies", "carpenter's tools", "cobbler's tools",
    "cook's utensils", "glassblower's tools", "jeweler's tools",
    "leatherworker's tools", "mason's tools", "painter's supplies",
    "potter's tools", "smith's tools", "tinker's tools", "weaver's tools",
    "woodcarver's tools", "gaming set", "musical instrument",
  ];
  ...
  List<String> _extractWeaponProfsFromTraits(String traits) {
    ...
    for (final match in pattern.allMatches(lower)) {
      ...
      if (found.contains('weapon') || _knownWeapons.any((w) => found.contains(w))) {
        final clean = found.replaceAll(RegExp(r'\s+'), ' ').replaceAll(RegExp(r'[*_]'), '').trim();
        for (final item in clean.split(RegExp(r',| and '))) {
          final t = item.trim();
          if (t.isNotEmpty && (_knownWeapons.any((w) => t.contains(w)) || t.contains('weapon'))) {
  ...
  List<String> _extractSkillProfsFromTraits(String traits) {
    ...
    for (final match in pattern1.allMatches(traits)) {
      ...
      if (CharacterSkillViewModel.allDndSkills
          .any((s) => s.toLowerCase() == found.toLowerCase())) {
  ...
  List<String> _extractToolProfsFromTraits(String traits) {
    final tools = <String>{};
    final lower = traits.toLowerCase();
    final pattern = RegExp(r'proficiency with (?:the )?([^.\n]+)', caseSensitive: false);
    for (final match in pattern.allMatches(lower)) {
      final found = match.group(1) ?? '';
